...
// 1. FIXED: Hardcoded costs - Now uses database (Item table, PurchaseLog, SystemSettings)
//    - Shawarma cost per kg: Dynamic lookup instead of hardcoded 12.35 QAR
//    - Inventory costs: Retrieved from Item table instead of hardcoded arrays
//    - Added fallback to legacy values with warnings if database is empty
//
// 2. FIXED: Food cost calculation - Now calculates ACTUAL cost from inventory usage
//    - Previous: Estimated as 22% of revenue (completely inaccurate)
//    - Current: Calculates from actual inventory consumption (opening + received - remaining - expired)
//    - Includes shawarma stack usage cost
//    - Falls back to estimation only if calculation fails
//
// 3. FIXED: Data loss during migration - Sales breakdown data now preserved
//    - Previous: Set cash_sales, card_sales, delivery amounts to 0
//    - Current: Retrieves from DailySalesBreakdown and DailyPettyCash tables
//    - Preserves aggregator details and petty cash entries
//
// 4. ADDED: Rollback mechanism for migrations
//    - rollbackMigrationForDate() - Roll back single date
//    - rollbackMigrationDateRange() - Roll back date range
//    - verifyOldDataIntegrity() - Verify data before rollback
//
// 5. ADDED: Batch operations for better performance
//    - batchAppendRows() - Add multiple rows at once
//    - batchDeleteRows() - Delete rows in proper order
//    - Reduces Google Apps Script API calls significantly
//
// 6. ADDED: Helper functions for cost management
//    - getShawarmaStackCostPerKg() - Dynamic shawarma cost lookup
//    - getSystemSetting() - Retrieve system-wide settings
//    - getMostRecentPurchase() - Get latest purchase price
//    - getItemCostFromDatabase() - Get item cost from multiple sources
//    - initializeItemCostsFromLegacy() - Migrate hardcoded costs to database
//
// === REMAINING ISSUES TO ADDRESS ===
// 1. DailySales table name collision - exists in both old and new schemas
//    RECOMMENDATION: Keep as shared table for migration compatibility
//
// 2. Consider adding transaction-like behavior for critical operations
// 3. Add more comprehensive error recovery mechanisms
// 4. Implement migration progress tracking UI
//
...
// Cost management functions - retrieve costs from database instead of hardcoding
function getShawarmaStackCostPerKg(date) {
  // Try to get from system settings first
  const settings = getSystemSetting('shawarma_cost_per_kg');
  if (settings) {
    return parseFloat(settings) || 12.35; // fallback to default if parse fails
  }

  // Try to get from purchase log based on most recent purchase
  const purchaseData = getMostRecentPurchase('Chicken Shawarma', date);
  if (purchaseData && purchaseData.cost_per_unit) {
    return parseFloat(purchaseData.cost_per_unit);
  }

  // Final fallback - use default but log warning
  console.log('WARNING: Using default shawarma cost. Please update SystemSettings or PurchaseLog');
  return 12.35;
}

function getSystemSetting(settingName) {
  try {
    const settings = getSheetData('SystemSettings');
    if (!settings || !Array.isArray(settings)) return null;

    const setting = settings.find(s => s.setting_name === settingName);
    return setting ? setting.setting_value : null;
  } catch (error) {
    console.log('Error getting system setting: ' + error.message);
    return null;
  }
}

function getMostRecentPurchase(itemName, beforeDate) {
  try {
    const purchases = getSheetData('PurchaseLog');
    if (!purchases || !Array.isArray(purchases)) return null;

    const targetDate = new Date(beforeDate);
    const relevantPurchases = purchases.filter(p => {
      if (!p.delivery_date) return false;
      const purchaseDate = new Date(p.delivery_date);
      return purchaseDate <= targetDate;
    });

    relevantPurchases.sort((a, b) => new Date(b.delivery_date) - new Date(a.delivery_date));

    return relevantPurchases[0] || null;
  } catch (error) {
    console.log('Error getting recent purchase: ' + error.message);
    return null;
  }
}

function getItemCostFromDatabase(itemId, date) {
  try {
    // First, try to get from Item table
    const items = getSheetData('Item');
    if (items && Array.isArray(items)) {
      const item = items.find(i => i.id === itemId);
      if (item && item.cost_per_unit && parseFloat(item.cost_per_unit) > 0) {
        return parseFloat(item.cost_per_unit);
      }
    }

    // If not found or zero, try to get from purchase log
    const purchases = getSheetData('PurchaseLog');
    if (purchases && Array.isArray(purchases)) {
      const targetDate = new Date(date);
      const itemPurchases = purchases.filter(p => {
        return p.item_id === itemId &&
               p.delivery_date &&
               new Date(p.delivery_date) <= targetDate;
      });

      if (itemPurchases.length > 0) {
        itemPurchases.sort((a, b) => new Date(b.delivery_date) - new Date(a.delivery_date));
        return parseFloat(itemPurchases[0].cost_per_unit) || 0;
      }
    }

    return 0;
  } catch (error) {
    console.log('Error getting item cost from database: ' + error.message);
    return 0;
  }
}

// Helper function to initialize Item table with legacy costs
function initializeItemCostsFromLegacy() {
  try {
    const items = getSheetData('Item');
    if (!items || !Array.isArray(items) || items.length === 0) {
      console.log('Item table is empty. Please run initializeLegacyMappings first.');
      return { success: false, message: 'Item table is empty' };
    }
    ...
    const legacyKeyIndex = headers.indexOf('legacy_key');
    const categoryIndex = headers.indexOf('category');

    if (costIndex === -1 || legacyKeyIndex === -1 || categoryIndex === -1) {
    ...
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      const currentCost = parseFloat(row[costIndex]) || 0;

      // Only update if cost is 0 or empty
      if (currentCost === 0) {
        const legacyKey = row[legacyKeyIndex];
        const category = row[categoryIndex];

        if (legacyKey && category) {
          const newCost = getItemUnitCostFallback(category, legacyKey);
          if (newCost > 0) {
            updates.push({
              row: i + 1,
              cost: newCost,
    ...
    updates.forEach(update => {
      itemSheet.getRange(update.row, costIndex + 1).setValue(update.cost);
      updated++;
    });
    ...
    return {
      success: true,
      updated: updated,
      message: `Updated ${updated} item costs`
    };
  } catch (error) {
    console.log('Error initializing item costs: ' + error.message);
...
function batchAppendRows(sheetName, rows) {
  ...
  try {
    ...
    if (!sheet) {
      console.log(`Sheet ${sheetName} not found for batch append`);
      return;
    }

    // Append all rows at once using setValues
    ...
    console.log(`Batch append error for ${sheetName}: ${error.message}`);
    throw error;
  }
}
...
function getItemUnitCost(category, itemName) {
  // DEPRECATED: Use getItemCostFromDatabase instead
  // This function now uses database lookups instead of hardcoded values

  try {
    const items = getSheetData('Item');
    if (!items || !Array.isArray(items)) {
      return getItemUnitCostFallback(category, itemName);
    }

    // Try to find item by category and name
    const item = items.find(i => {
      const categoryMatch = i.category && i.category.toLowerCase().includes(category.toLowerCase());
      const nameMatch = i.name && (
        i.name.toLowerCase() === itemName.replace(/_/g, ' ').toLowerCase() ||
        i.legacy_key === itemName
      );
    ...
    // Fallback to legacy hardcoded values with warning
    console.log(`WARNING: Using fallback cost for ${category}/${itemName}. Please update Item table.`);
    return getItemUnitCostFallback(category, itemName);
  } catch (error) {
    console.log('Error getting item unit cost: ' + error.message);
    return getItemUnitCostFallback(category, itemName);
  }
}

function getItemUnitCostFallback(category, itemName) {
  // Legacy fallback costs - should be moved to database
  const costs = {
    rawProteins: {
...
function saveSalesData(entryData, entryDate, employeeId, options = {}) {
  ...
  try {
    ...
    if (actualFoodCost > 0) {
      ...
      actualFoodCost = totalRevenue * 0.22;
      foodCostPercentage = 22.0;
    }
  } catch (error) {
    console.log(`ERROR calculating food cost: ${error.message}. Using estimation.`);
    actualFoodCost = totalRevenue * 0.22;
    foodCostPercentage = 22.0;
  }
...
function calculateActualFoodCost(entryData, entryDate) {
  ...
  const inventoryCategories = [
    { data: entryData.rawProteins, category: 'rawProteins' },
    { data: entryData.marinatedProteins, category: 'marinatedProteins' },
    { data: entryData.bread, category: 'bread' },
    { data: entryData.highCostItems, category: 'highCostItems' }
  ];
  ...
  inventoryCategories.forEach(cat => {
    ...
    for (const key in cat.data) {
      if (cat.data.hasOwnProperty(key)) {
        // Calculate usage: opening + received - remaining - expired
        const opening = parseFloat(cat.data[key.replace(/_remaining|_received|_expired/, '_opening')]) || 0;
        const received = parseFloat(cat.data[key.replace(/_remaining|_opening|_expired/, '_received')]) || 0;
        const expired = parseFloat(cat.data[key.replace(/_remaining|_received|_opening/, '_expired')]) || 0;
        const remaining = parseFloat(cat.data[key.replace(/_opening|_received|_expired/, '_remaining')]) || 0;

        // Only calculate for _remaining fields to avoid counting same item multiple times
        if (key.endsWith('_remaining')) {
...
// CRITICAL FIX: Rollback mechanism for migrations
function rollbackMigrationForDate(dateString, options = {}) {
  try {
    logMigrationActivity('rollback_start', { date: dateString }, 'info');

    // 1. Verify old data still exists
    const oldData = generateReportFromOldTables(dateString);
    if (!oldData || !oldData.dataFound) {
      return {
        success: false,
        message: 'Cannot rollback - old data not found',
        date: dateString
      };
    }

    // 2. Delete new table entries for this date
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const targetDate = new Date(dateString).toDateString();
    const newTablesToClean = ['SnapshotLog', 'PettyCashDetail'];

    newTablesToClean.forEach(tableName => {
      ...
      logMigrationActivity('rollback_table_cleaned', {
        table: tableName,
        rows_deleted: rowsToDelete.length
      }, 'info');
    });

    logMigrationActivity('rollback_complete', {
      date: dateString,
      success: true
    }, 'success');
    ...
    logMigrationActivity('rollback_failed', {
      date: dateString,
      error: error.message
    }, 'error');
...
function rollbackMigrationDateRange(startDate, endDate, options = {}) {
  const dates = getHistoricalDataDates(startDate, endDate);
  const results = {
    total: dates.length,
    successful: 0,
    failed: 0,
  ...
  dates.forEach(date => {
    const result = rollbackMigrationForDate(date, options);
    if (result.success) {
      results.successful++;
    } else {
      results.failed++;
...
function verifyOldDataIntegrity(dateString) {
  // Verify that old table data is complete and valid
  try {
    const oldData = generateReportFromOldTables(dateString);
    if (!oldData || !oldData.dataFound) {
      return { valid: false, message: 'No data found in old tables' };
    }

    const issues = [];

    // Check for required fields
    if (!oldData.sales || !oldData.sales.total_revenue) {
      issues.push('Missing sales data');
    }

    if (!oldData.shawarma) {
      issues.push('Missing shawarma data');
    }
...
function convertOldDataToNewFormat(oldData) {
  ...
  // CRITICAL FIX: Retrieve actual sales breakdown data instead of setting to 0
  let salesBreakdown = {
    cash_sales: 0,
    card_sales: 0,
    delivery_sales: 0,
  ...
  try {
    // Try to get sales breakdown from DailySalesBreakdown table
    const breakdownData = getSalesBreakdownForDate(oldData.date);
    if (breakdownData) {
      salesBreakdown.cash_sales = parseFloat(breakdownData.cash_sales) || 0;
      salesBreakdown.card_sales = parseFloat(breakdownData.card_sales) || 0;
      salesBreakdown.delivery_sales = parseFloat(breakdownData.delivery_sales) || 0;
      salesBreakdown.cash_expenses = parseFloat(breakdownData.cash_expenses) || 0;

      // Parse aggregator details if available
      if (breakdownData.aggregator_details) {
        try {
          const aggregators = JSON.parse(breakdownData.aggregator_details);
          if (Array.isArray(aggregators)) {
            salesBreakdown.delivery_aggregators = aggregators;
          }
        } catch (e) {
          console.log('Could not parse aggregator details for ' + oldData.date);
        }
      }
    }

    // Try to get petty cash details from DailyPettyCash table
    pettyCashEntries = getPettyCashEntriesForDate(oldData.date);

  } catch (error) {
    console.log('Error retrieving sales breakdown during migration: ' + error.message);
  }
  ...
  const convertedData = {
    ...
    paymentBreakdown: salesBreakdown,
...
function getSalesBreakdownForDate(dateString) {
  try {
    const data = getSheetData('DailySalesBreakdown');
    if (!data || !Array.isArray(data)) return null;

    const targetDate = new Date(dateString).toDateString();
    const breakdown = data.find(row => {
      if (!row.sales_date) return false;
      return new Date(row.sales_date).toDateString() === targetDate;
    });

    return breakdown || null;
  } catch (error) {
    console.log('Error getting sales breakdown: ' + error.message);
    return null;
  }
}

function getPettyCashEntriesForDate(dateString) {
  try {
    const data = getSheetData('DailyPettyCash');
    ...
    console.log('Error getting petty cash entries: ' + error.message);
    return [];
  }
}
